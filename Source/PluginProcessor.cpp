...
RhythmEngineProcessor::RhythmEngineProcessor()
    : juce::AudioProcessor(BusesProperties().withOutput(
      ...
      builder_(
          std::make_unique<rhythm::TrackBuilder>(120.0, transport_.get())) {
    audio_->open();
}
...
void RhythmEngineProcessor::prepareToPlay(double sampleRate,
                                          int samplesPerBlock) {
    audio_->prepareToPlay(sampleRate, samplesPerBlock);
}

void RhythmEngineProcessor::releaseResources() { audio_->releaseResources(); }

bool RhythmEngineProcessor::isBusesLayoutSupported(
...
void RhythmEngineProcessor::processBlock(juce::AudioBuffer<float> &buffer,
                                         juce::MidiBuffer &) {
...
void RhythmEngineProcessor::getStateInformation(juce::MemoryBlock &destData) {
    const std::string json =
        rhythm::ProjectSerializer::serialize(builder_->state(), projectName_);
    destData.append(json.data(), json.size());
}

void RhythmEngineProcessor::setStateInformation(const void *data,
                                                int sizeInBytes) {
    const std::string json(static_cast<const char *>(data),
                           (size_t)sizeInBytes);
