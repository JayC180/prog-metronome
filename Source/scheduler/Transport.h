...
class Transport {
  public:
    ...
    // Swap in a new set of interpret results - automatically stops +
    // reconfigures.
    ...
    // Optional UI hook - invoked for every fired event on the dispatcher
    // thread.
    void setOnFired(std::function<void(const ScheduledEvent &)> cb) {
        onFired_ = std::move(cb);
    }
