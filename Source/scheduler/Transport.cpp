...
Transport::Transport(AudioEngine *engine)
    : audioEngine_(engine), soundMap_(std::make_shared<SoundMap>()) {}
...
void Transport::setTrackFlags(const std::string &trackId, bool muted,
                              bool soloed) {
    std::lock_guard<std::mutex> lock(resultsMutex_);
    for (auto &r : results_) {
...
void Transport::runScheduler() {
    while (isPlaying_.load()) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(SCHEDULER_INTERVAL_MS));
    }
}
    ...
    while (isPlaying_.load()) {
...

        {
            ...
            while (!eventQueue_.empty() &&
                   eventQueue_.top().absoluteNanos <= now) {
        ...
        for (const auto &ev : toFire) {
            ...
            const bool audible = anySoloed
                                     ? (flags != nullptr && flags->soloed)
        ...
        std::this_thread::sleep_for(
            std::chrono::nanoseconds(DISPATCHER_SPIN_NANOS));
    }
}
