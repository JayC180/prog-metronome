...
StreamClock::StreamClock(InterpretResult result) : result_(std::move(result)) {}

void StreamClock::fillUntil(int64_t horizonNanos, int64_t originNanos,
                            EventQueue &queue) {
    if (!isRunning())
        return;

    // Hard upper bound to avoid pathological infinite loops in case both lists
    // are empty.
    int guard = 500'000;
    ...
    while (guard-- > 0) {
        ...
        switch (phase_) {
        case Phase::Prefix:
            current = &result_.events;
            break;
        case Phase::Infinite:
            current = &result_.infinitePassEvents;
            break;
        case Phase::Stopped:
            return;
        }
        ...
        if (current->empty()) {
            ...
            if (phase_ == Phase::Stopped)
                return;
            // If both phases are empty we would spin forever; the guard catches
            // that.
            continue;
        }
        ...
        const int64_t fireNanos =
            originNanos + loopOriginNanos_ + event.offsetNanos;

        if (fireNanos >= horizonNanos)
            return;
...
void StreamClock::advancePhase() {
    ...
    case Phase::Prefix:
        ...
        phase_ = (!result_.infinitePassEvents.empty()) ? Phase::Infinite
                                                       : Phase::Prefix;
        break;
    case Phase::Infinite:
...
std::string StreamClock::debugState() const {
    ...
    os << "StreamClock(" << result_.trackId << " phase=" << phase
       << " idx=" << eventIndex_ << " loopOrigin=" << loopOriginNanos_ << "ns"
       << " fired=" << firedTotal_ << ")";
    return os.str();
}
