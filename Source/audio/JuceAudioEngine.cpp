...
JuceAudioEngine::JuceAudioEngine() : soundMap_(std::make_shared<SoundMap>()) {
...
bool JuceAudioEngine::loadSample(const std::string &soundId,
...
void JuceAudioEngine::trigger(const std::string &soundId, float volume,
                              int64_t /*expectedFireNanos*/) {
    int start1 = 0, size1 = 0, start2 = 0, size2 = 0;
    triggerFifo_.prepareToWrite(1, start1, size1, start2, size2);
    if (size1 + size2 == 0)
        return; // queue full - drop trigger
...
void JuceAudioEngine::prepareToPlay(double sampleRate,
                                    int /*samplesPerBlock*/) {
...
void JuceAudioEngine::generateBuiltInClicks(double sampleRate) {
    auto makeClick = [sampleRate](double freqHz, double decay, int lengthMs,
                                  float gain) {
...
void JuceAudioEngine::releaseResources() { stopAll(); }
...
void JuceAudioEngine::startVoice(const Trigger &t) {
    ...
    for (int i = 0; i < (int)voices_.size(); ++i)
        if (!voices_[(size_t)i].active.load()) {
            slot = i;
            break;
        }
    if (slot < 0)
    ...
    v.playbackRatio =
        (sample->sourceSampleRate / deviceSampleRate_.load()) * sample->pitch;
    v.active.store(true);
}
...
void JuceAudioEngine::processBlock(juce::AudioBuffer<float> &buffer) {
    ...
    for (int i = 0; i < size1; ++i)
        startVoice(triggerBuffer_[(size_t)(start1 + i)]);
    for (int i = 0; i < size2; ++i)
        startVoice(triggerBuffer_[(size_t)(start2 + i)]);
    triggerFifo_.finishedRead(ready);
...
void JuceAudioEngine::render(juce::AudioBuffer<float> &buffer) {
    ...
    for (auto &v : voices_) {
        ...
        for (int n = 0; n < numSamples; ++n) {
            if (pos >= (double)sourceSamples - 1.0) {
                v.active.store(false);
                break;
            }
