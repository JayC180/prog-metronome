...
std::optional<TrackItem> varToItem(const juce::var &v) {
    ...
    if (type == "beat") {
        if (!obj->hasProperty("num") || !obj->hasProperty("den"))
            return std::nullopt;
        TrackItem::Beat b;
        b.displayNum = (int)obj->getProperty("num");
        b.displayDenom = (int)obj->getProperty("den");
        if (obj->hasProperty("active"))
            b.active = (bool)obj->getProperty("active");
        if (obj->hasProperty("soundId"))
    ...
    if (type == "open")
        return TrackItem(TrackItem::BracketOpen{});
    if (type == "close")
        return TrackItem(TrackItem::BracketClose{});
    if (type == "repeat") {
...
std::string ProjectSerializer::serialize(const TrackBuilderState &state,
...
TrackBuilderState
ProjectSerializer::deserialize(const std::string &jsonString) {
    juce::var v;
    ...
    auto *obj = v.getDynamicObject();
    if (obj == nullptr)
        throw DeserializationException("Empty document");
    ...
    if (version != 1)
        throw DeserializationException("Unsupported version " +
    ...
    if (auto *tracksArr = tracksVar.getArray()) {
        ...
        for (int i = 0; i < tracksArr->size(); ++i) {
            ...
            d.muted = tobj->hasProperty("muted")
                          ? (bool)tobj->getProperty("muted")
                          : false;
            d.soloed = tobj->hasProperty("soloed")
                           ? (bool)tobj->getProperty("soloed")
                           : false;
            ...
            if (auto *itemsArr = tobj->getProperty("items").getArray()) {
                ...
                for (int j = 0; j < itemsArr->size(); ++j)
                    if (auto it = varToItem((*itemsArr)[j]))
    ...
    if (st.tracks.empty())
        st.tracks.push_back(TrackBuilder::newTrackDraft(0));
    return st;
}

std::optional<TrackBuilderState>
ProjectSerializer::deserializeOrNull(const std::string &jsonString) {
    try {
...
ProjectSerializer::peekName(const std::string &jsonString) {
    juce::var v;
    if (juce::JSON::parse(juce::String(jsonString), v).failed())
        return std::nullopt;
    auto *obj = v.getDynamicObject();
...
std::optional<double>
ProjectSerializer::peekBpm(const std::string &jsonString) {
    juce::var v;
    if (juce::JSON::parse(juce::String(jsonString), v).failed())
        return std::nullopt;
    auto *obj = v.getDynamicObject();
    if (obj == nullptr || !obj->hasProperty("bpm"))
        return std::nullopt;
