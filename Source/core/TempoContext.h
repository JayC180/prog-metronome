...
class TempoContext {
  public:
    TempoContext(double bpm, Rational pulseUnit = Rational::ONE,
    ...
    double effectiveBpm() const {
        return bpm_ / (pulseUnit_ * modulation_).toDouble();
    }
    ...
    int64_t tickOffsetToNanos(int64_t ticks) const {
        return (int64_t)((double)pulseNanos() * (double)ticks /
                         (double)TICKS_PER_PULSE);
    }
...
// Pulse-unit presets - referenced by upper layers but the engine doesn't
// enforce them.
namespace PulseUnit {
