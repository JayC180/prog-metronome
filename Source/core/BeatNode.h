  ...
  public:
    ...
    static BeatNodePtr active(Rational dur, std::string sound) {
        return std::make_shared<LeafNode>(
            dur, std::optional<std::string>(std::move(sound)));
    }
...
class GroupNode final : public BeatNode {
  public:
    GroupNode(Rational dur, int divisions, std::vector<BeatNodePtr> children,
              std::optional<std::string> label = std::nullopt)
        : duration_(dur), divisions_(divisions), children_(std::move(children)),
    ...
    Rational slotDuration() const {
        return duration_ / Rational((int64_t)divisions_, 1);
    }
...
class SequenceNode final : public BeatNode {
  public:
    ...
    void flattenInto(const Rational &parentOffset,
        ...
        Rational cursor = parentOffset;
        for (auto &n : nodes_) {
            n->flattenInto(cursor, out);
            cursor += n->duration();
        }
    }
    void validateInto(std::vector<ValidationWarning> &out) const override {
        if (isEmpty())
            out.push_back({this, "Empty sequence", Severity::Warning});
        for (auto &n : nodes_)
            n->validateInto(out);
    }
