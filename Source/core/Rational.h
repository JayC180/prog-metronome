...
// Every subdivision used in the engine converts to ticks with zero rounding
// error.
inline constexpr int64_t TICKS_PER_PULSE = 55440;

// Immutable rational number - the only numeric type for durations inside the
// engine. No floating point until the audio boundary (TempoContext::toNanos).
...
class Rational {
  public:
    ...
    static constexpr Rational of(int n, int d) {
        return Rational((int64_t)n, (int64_t)d);
    }
    static constexpr Rational of(int64_t n, int64_t d) {
        return Rational(n, d);
    }
    static constexpr Rational ofWhole(int whole) {
        return Rational((int64_t)whole, 1);
    }
    ...
    Rational operator+(const Rational &o) const {
        return Rational(num_ * o.den_ + o.num_ * den_, den_ * o.den_).reduced();
    }
    Rational operator-(const Rational &o) const {
        return Rational(num_ * o.den_ - o.num_ * den_, den_ * o.den_).reduced();
    }
    Rational operator*(const Rational &o) const {
        return Rational(num_ * o.num_, den_ * o.den_).reduced();
    }
    Rational operator/(const Rational &o) const {
        if (o.num_ == 0)
    ...
    // Integer division to ticks - zero rounding error for any subdivision in
    // the supported set.
