...
void ChipButton::resolveColours(juce::Colour &bg, juce::Colour &border,
...
void ChipButton::mouseDown(const juce::MouseEvent &) {
    if (enabledLook_ && onClick_)
        onClick_();
}

// ----- NumKeyButton -----

NumKeyButton::NumKeyButton(juce::String label) : label_(std::move(label)) {
    setMouseCursor(juce::MouseCursor::PointingHandCursor);
}
...
void TrackChip::paint(juce::Graphics &g) {
    const auto r = getLocalBounds().toFloat().reduced(0.5f);
    const auto bg =
        active_ ? activeColour_.withAlpha(0.15f) : RhythmColors::bg3();
    const auto border =
        active_ ? activeColour_.withAlpha(0.4f) : RhythmColors::border1();
