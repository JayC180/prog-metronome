...
juce::Typeface::Ptr
RhythmLookAndFeel::getTypefaceForFont(const juce::Font &font) {
#ifdef RHYTHM_HAS_EMBEDDED_FONT
    if (font.isBold() && boldTypeface_ != nullptr)
        return boldTypeface_;
    if (regularTypeface_ != nullptr)
...
void RhythmLookAndFeel::drawLabel(juce::Graphics &g, juce::Label &label) {
    ...
    if (!label.isBeingEdited()) {
        g.setColour(label.findColour(juce::Label::textColourId)
                        .withAlpha(label.isEnabled() ? 1.0f : 0.5f));
        g.setFont(
            juce::Font(juce::FontOptions((float)label.getFont().getHeight())));
        g.drawFittedText(
            label.getText(), label.getLocalBounds(),
...
void RhythmLookAndFeel::drawLinearSlider(juce::Graphics &g, int x, int y, int w,
                                         int h, float sliderPos,
                                         float /*minPos*/, float /*maxPos*/,
                                         juce::Slider::SliderStyle,
                                         juce::Slider &s) {
    const auto fullBounds =
        juce::Rectangle<float>((float)x, (float)y, (float)w, (float)h);
    const float trackY = fullBounds.getCentreY() - 2.0f;
    const auto trackBg = juce::Rectangle<float>(fullBounds.getX(), trackY,
                                                fullBounds.getWidth(), 4.0f);
    g.setColour(RhythmColors::bg3());
    g.fillRoundedRectangle(trackBg, 2.0f);

    const auto trackFg = juce::Rectangle<float>(
        fullBounds.getX(), trackY, sliderPos - fullBounds.getX(), 4.0f);
    g.setColour(s.isEnabled() ? RhythmColors::accent()
                              : RhythmColors::textDim());
    g.fillRoundedRectangle(trackFg, 2.0f);

    g.setColour(s.isEnabled() ? RhythmColors::thumbColor()
                              : RhythmColors::textDim());
    g.fillEllipse(juce::Rectangle<float>(
        sliderPos - 6.0f, fullBounds.getCentreY() - 6.0f, 12.0f, 12.0f));
}

void RhythmLookAndFeel::fillTextEditorBackground(juce::Graphics &g, int w,
                                                 int h, juce::TextEditor &) {
    g.setColour(RhythmColors::bg2());
    g.fillRoundedRectangle(0.0f, 0.0f, (float)w, (float)h, 4.0f);
}

void RhythmLookAndFeel::drawTextEditorOutline(juce::Graphics &g, int w, int h,
                                              juce::TextEditor &te) {
    g.setColour(te.hasKeyboardFocus(false) ? RhythmColors::accent()
                                           : RhythmColors::border1());
    g.drawRoundedRectangle(0.5f, 0.5f, (float)w - 1.0f, (float)h - 1.0f, 4.0f,
                           1.0f);
}
