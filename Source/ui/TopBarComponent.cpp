...
TopBarComponent::TopBarComponent(TrackBuilder &builder) : builder_(builder) {
...
void TopBarComponent::paintBpmCard(juce::Graphics &g) {
    ...
    const auto border =
        playing ? RhythmColors::cautionBorder() : RhythmColors::border1();
    const auto bpmCol =
        playing ? RhythmColors::caution() : RhythmColors::textPrimary();
...
void TopBarComponent::paintPlayStopButton(juce::Graphics &g) {
    ...
    const bool playing = builder_.state().isPlaying();
    const auto bg =
        playing ? RhythmColors::dangerBg() : RhythmColors::accentBg();
    const auto border =
        playing ? RhythmColors::dangerBorder() : RhythmColors::accentBorder();
    g.setColour(bg);
    g.fillRoundedRectangle(r, 6.0f);
    g.setColour(border);
    g.drawRoundedRectangle(r, 6.0f, 1.0f);
    ...
    if (playing) {
        ...
        const auto sq = juce::Rectangle<float>(
            (float)playStopArea_.getCentreX() - 5.0f,
            (float)playStopArea_.getCentreY() - 5.0f, 10.0f, 10.0f);
...
void TopBarComponent::mouseDown(const juce::MouseEvent &e) {
    ...
    if (projectNameArea_.contains(e.getPosition())) {
        if (onProjectNameClicked)
            onProjectNameClicked();
        return;
    }
