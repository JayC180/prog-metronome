...
class TrackRowComponent::ItemStrip : public juce::Component {
  public:
    ...
    void syncToState() {
        ...
        if (st.isPlaying())
            autoScrollTo(lastPlayingIndex_);
        else if (!st.cursorIndex.has_value())
            autoScrollTo((int)draft->items.size() - 1);
        else if (st.activeTrackIndex == trackIndex_)
            autoScrollTo(*st.cursorIndex);
    }
  ...
  private:
    ...
    void layoutItems() {
        ...
        // Rebuild child list only when structure changes (count or item kinds
        // differ).
    ...
    void autoScrollTo(int idx) {
        ...
        const int target =
            juce::jmax(0, comp.getX() + comp.getWidth() / 2 - viewW / 2);
