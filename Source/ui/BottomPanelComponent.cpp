...
BottomPanelComponent::BottomPanelComponent(TrackBuilder &builder)
    : builder_(builder), navPrevButton_("<-"),
      ...
      mmButton_("mm"), setBpmButton_("=bpm"), navNextButton_("->") {
    ...
    addToolButton(mmButton_, [this] {
        if (onMmRequested)
            onMmRequested();
    });
    addToolButton(setBpmButton_, [this] {
        if (onSetBpmRequested)
            onSetBpmRequested();
    });
    ...
    changeSoundButton_.setColours(RhythmColors::accentBg(),
                                  ...
                                  RhythmColors::accent());
    changeSoundButton_.setOnClick([this] {
        if (onChangeBeatSound)
            onChangeBeatSound();
    });
...
void BottomPanelComponent::rebuildToolbar() {
    const auto &s = builder_.state();
    denomButton_.setLabel(juce::String::fromUTF8(u8"÷") +
                          juce::String(s.activeDenom()));
    denomButton_.setStateColor(s.isDenomMode()
                                   ? ChipButton::StateColor::Editing
                                   : ChipButton::StateColor::Normal);
    denomButton_.setEnabledLook(s.canEdit());

    openBracketButton_.setStateColor(s.canOpenBracket()
                                         ? ChipButton::StateColor::Normal
                                         : ChipButton::StateColor::Dim);
    openBracketButton_.setEnabledLook(s.canOpenBracket());

    closeBracketButton_.setStateColor(s.canCloseBracket()
                                          ? ChipButton::StateColor::Normal
                                          : ChipButton::StateColor::Dim);
    closeBracketButton_.setEnabledLook(s.canCloseBracket());

    if (s.isRepeatMode())
        repeatButton_.setStateColor(ChipButton::StateColor::Editing);
    else if (s.canSetRepeat())
        repeatButton_.setStateColor(ChipButton::StateColor::Normal);
    else
        repeatButton_.setStateColor(ChipButton::StateColor::Dim);
    ...
    setBpmButton_.setStateColor(s.canInsertTempo()
                                    ? ChipButton::StateColor::Normal
                                    : ChipButton::StateColor::Dim);
...
void BottomPanelComponent::rebuildNumpad() {
    ...
    juce::String hint;
    if (s.isDenomMode())
        hint = "set subdivision - tap or use custom";
    else if (s.isRepeatMode())
        hint = juce::String::fromUTF8(u8"set repeat (1-9) or custom / ∞");
    else if (s.isEditMode())
        hint = "edit value - tap a number or use custom";
    hintLabel_.setText(hint, juce::dontSendNotification);

    const bool customEnabled = s.isRepeatMode() || s.isEditMode() ||
                               s.isDenomMode() ||
                               (item != nullptr && item->isBeat());
    customKey_.setColours(RhythmColors::bg3(), RhythmColors::border1(),
                          customEnabled ? RhythmColors::textPrimary()
                                        : RhythmColors::textMuted());

    if (s.isEditMode())
        editKey_.setColours(RhythmColors::cautionBg(),
                            RhythmColors::cautionBorder(),
                            RhythmColors::caution());
    else if (s.canEditItem())
        editKey_.setColours(RhythmColors::accentBg(),
                            RhythmColors::accentBorder(),
                            RhythmColors::accent());
    else
        editKey_.setColours(RhythmColors::bg3(), RhythmColors::border0(),
                            RhythmColors::textDim());

    backspaceKey_.setColours(RhythmColors::bg3(), RhythmColors::border1(),
                             RhythmColors::textMuted());
    for (auto &k : numKeys_)
        k->setColours(RhythmColors::bg3(), RhythmColors::border1(),
                      enabled ? RhythmColors::thumbColor()
                              : RhythmColors::textDim());
    ...
    if (enabled && beatSelected) {
        ...
        onOffStatus_.setColours(
            active ? RhythmColors::accentBg() : RhythmColors::bg3(),
        ...
        onOffStatus_.setLabel(!enabled ? "select a beat"
                                       : juce::String::fromUTF8(u8"—"));
        onOffStatus_.setColours(RhythmColors::bg3(), RhythmColors::border0(),
                                RhythmColors::textDim());
    ...
    if (beatSelected) {
        ...
        soundValueLabel_.setText(b.soundId.has_value()
                                     ? juce::String(*b.soundId)
                                     : juce::String("-"),
                                 juce::dontSendNotification);
    } else {
...
void BottomPanelComponent::paint(juce::Graphics &g) {
    ...
    g.drawLine((float)toolbarArea_.getX(),
               (float)toolbarArea_.getBottom() - 0.5f,
               (float)toolbarArea_.getRight(),
               (float)toolbarArea_.getBottom() - 0.5f, 0.5f);
    g.drawLine((float)editArea_.getX(), (float)editArea_.getBottom() - 0.5f,
               (float)editArea_.getRight(), (float)editArea_.getBottom() - 0.5f,
               0.5f);
}
...
void BottomPanelComponent::resized() {
    ...
    toolbarArea_ = bounds.removeFromTop(50);
    {
        ...
        auto layout = [&r](ChipButton &b, int w) {
    ...
    numpadArea_ = bounds;
    {
        ...
        editKey_.setBounds(r.getX() + colWidth + 4, lastRowY, colWidth,
                           rowHeight);
        backspaceKey_.setBounds(r.getX() + 2 * (colWidth + 4), lastRowY,
                                colWidth, rowHeight);
    }
}
...
void BottomPanelComponent::onNumKey(int n) {
    ...
    if (s.isDenomMode())
        ...
        builder_.repeatDigit(n);
    else if (s.isEditMode())
        builder_.editDigit(n);
    else
...
void BottomPanelComponent::onCustom() {
    ...
    if (s.isEditMode()) {
        if (s.cursorItem() != nullptr) {
            if (s.cursorItem()->isRepeat() && onRepeatCustomRequested)
                onRepeatCustomRequested();
            else if (s.cursorItem()->isSetBpm() && onSetBpmRequested)
                onSetBpmRequested();
            else if (s.cursorItem()->isBeat() && onCustomBeatRequested)
                onCustomBeatRequested();
        }
        return;
    }
    if (s.isRepeatMode()) {
        if (onRepeatCustomRequested)
            onRepeatCustomRequested();
        return;
    }
    if (s.isDenomMode()) {
        if (onCustomDenomRequested)
            onCustomDenomRequested();
        return;
    }
    if (onCustomBeatRequested)
        onCustomBeatRequested();
}

void BottomPanelComponent::onEditToggle() {
    const auto *item = builder_.state().cursorItem();
    if (item != nullptr && item->isModulation()) {
        if (onMmRequested)
            onMmRequested();
        return;
    }
    if (item != nullptr && item->isSetBpm()) {
        if (onSetBpmRequested)
            onSetBpmRequested();
        return;
    }
