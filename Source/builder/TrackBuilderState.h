...
struct InputMode {
    ...
    static InputMode edit(bool firstDigit = true) {
        return {Kind::Edit, std::nullopt, firstDigit};
    }
};
...
class TrackBuilderState {
  public:
    ...
    const TrackDraft *activeTrack() const {
        if (activeTrackIndex < 0 || activeTrackIndex >= (int)tracks.size())
            return nullptr;
    ...
    bool isDenomMode() const {
        return inputMode.kind == InputMode::Kind::Denom;
    }
    bool isRepeatMode() const {
        return inputMode.kind == InputMode::Kind::RepeatCount;
    }
    ...
    double displayBpm() const {
        if (isPlaying() && runningBpm.has_value())
            return *runningBpm;
        return bpm;
    }
