  ...
  public:
    ...
    struct Beat {
        ...
        std::string label() const {
            if (displayDenom == 1)
                return std::to_string(displayNum);
            return std::to_string(displayNum) + "/" +
                   std::to_string(displayDenom);
        }
    ...
    struct Repeat {
        ...
        std::string toString() const {
            return isInfinite() ? "×∞" : "×" + std::to_string(count);
        }
    };
    ...
    using Storage = std::variant<Beat, BracketOpen, BracketClose, Repeat,
                                 Modulation, SetBpm>;
    ...
    bool isBracketOpen() const {
        return std::holds_alternative<BracketOpen>(storage_);
    }
    bool isBracketClose() const {
        return std::holds_alternative<BracketClose>(storage_);
    }
    bool isRepeat() const { return std::holds_alternative<Repeat>(storage_); }
    bool isModulation() const {
        return std::holds_alternative<Modulation>(storage_);
    }
