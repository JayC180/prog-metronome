...
bool TrackBuilderState::canCloseBracket() const {
    if (!canEdit() || activeTrack() == nullptr ||
        inputMode.kind != InputMode::Kind::Normal)
        return false;
    if (insertionBlockedByInfinite())
        return false;
    const auto &t = *activeTrack();
    if (t.items.empty())
        return false;
    const int prevIdx = insertionIndex() - 1;
    if (prevIdx >= 0 && prevIdx < (int)t.items.size() &&
        t.items[(size_t)prevIdx].isBracketOpen())
        return false;
...
bool TrackBuilderState::canEditItem() const {
    ...
    if (item == nullptr)
        return false;
    if (item->isBeat() || item->isModulation() || item->isSetBpm())
        return true;
...
std::optional<int> TrackBuilderState::findRelevantClose() const {
    ...
    if (cursorIndex.has_value()) {
        ...
        if (idx >= 0 && idx < (int)t.items.size() &&
            t.items[(size_t)idx].isBracketClose())
            return idx;
    }
...
bool TrackBuilderState::hasRepeatAfterClose(const TrackDraft &t,
                                            int closeIndex) const {
    int i = closeIndex + 1;
...
std::vector<std::string> TrackBuilderState::buildErrors() const {
    ...
    for (int ti = 0; ti < (int)tracks.size(); ++ti) {
        ...
        if (infIdx >= 0) {
            ...
            const auto open =
                (closeIdx >= 0) ? t.matchingOpenIndex(closeIdx) : std::nullopt;
            if (open.has_value()) {
                bool hasMm = false;
