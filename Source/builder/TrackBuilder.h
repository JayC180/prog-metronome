...
class TrackBuilder {
  public:
    ...
    void setOnStateChanged(StateListener cb) {
        onStateChanged_ = std::move(cb);
    }

    using BuildErrorListener =
        std::function<void(const std::vector<std::string> &)>;
    void setOnBuildErrors(BuildErrorListener cb) {
        onBuildErrors_ = std::move(cb);
    }

    using BuildResultsListener =
        std::function<void(const std::vector<InterpretResult> &)>;
    void setOnBuildResults(BuildResultsListener cb) {
        onBuildResults_ = std::move(cb);
    }

    const std::vector<InterpretResult> &lastBuildResults() const {
        return lastBuildResults_;
    }
    const std::vector<std::string> &lastBuildErrors() const {
        return lastBuildErrors_;
    }
  ...
  private:
    ...
    void
    updateActiveTrack(const std::function<TrackDraft(const TrackDraft &)> &fn);
